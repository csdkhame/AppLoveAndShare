...
$res[project] = $db->select_query("SELECT * FROM   web_carall  where drivername='".$user_id."'   order by id desc  ");
 ...
 $arr[type] = $db->fetch($res[type]);
	...
	<div style="margin-left:-5px; margin-top:-20px;" class="cirnumbershow<? 	if($arr[project][driver_complete]=="1"){ ?>ok<? } ?>" id="cir_<?=$arr[project][id];?>"  ><table width="100%"  border="0" cellspacing="1" cellpadding="1">
  <tr>
  ...
  <tr>
      ...
      <tbody>
        ...
        <tr>
          <td width="50%" height="50%" align="center" class="tool-td-chat"> 
             ...
             ?>
            <button type="button" class="btn btn-default "  id="use_car_<?=$arr[project][id];?>" onclick="use_often('<?=$arr[project][id];?>','<?=$type;?>');"  style="width:100%">
              <center>
   ...
   box-shadow: 0px 1px 5px #DADADA;  }
	.tdtable  td {height:26px;}
	 ...
	 $plate_color="FF0000"; } ?><table width="100%" cellpadding="1" cellspacing="2"  style="margin-top:-10px;  margin-right: 0px; margin-bottom:0px; margin-right:10px; " >
       <?
...
?>
  ...
  <td width="80" height="70" align="center" bgcolor="#<?=$plate_color?>" style="border: solid 2px; height:70px; color:#DADADA; padding:10px; padding-right:10px;border-radius:10px;"><font color="#<? if($arr[project][plate_color]=="Green"){
	 ...
	 echo "FFFFFF"; } ?>"  class="font-38"><b><? echo $arr[project][plate_num];?> <br> 
      ...
      <tr>
        <td width="80"  class="font-22"><strong>ประเภท</strong></td>
	...
	function use_often(id,type){
		if(type=="open"){
		 ...
		 swal({
		  ...
		  var url = "popup.php?name=car&file=savedata&use_often="+type+"&id="+id;
//		 alert(url);
//		 $('#show_car_detail').load(url_<?=$arr[project][id]?>); 
