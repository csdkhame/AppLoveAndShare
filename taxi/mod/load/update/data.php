...
<?
///echo $arr[web_user][alert_new_work];


 if($arr[web_user][alert_new_work]==1) {
...
///  $("#alert_auto_new_work").click();
  ...
  <? if($arr[web_user][alert_cancel_work]==1) {
 ...
 $("#popup_alert_cancel_work").modal();
  document.getElementById('check_status_cancel_work').value ='1';
   ...
   <? } ?>
     ...
     <? if($arr[web_user][alert_lab_call]==1) {	 
...
var url="load_auto.php?name=load/update&file=control&sound_alert=checkin&terminal=<?=$arr[web_user][alert_lab_call_terminal];?>";
   $("#load_sound_lab_checkin").load(url);
   
var url_lab="load_auto.php?name=load/update&file=terminal&terminal=<?=$arr[web_user][alert_lab_call_terminal];?>";
$("#terminal_lab").load(url_lab); 
   ...
   <? } ?>
      ...
      <? if($arr[web_user][alert_lab_call]==2) {
...
var url="load_auto.php?name=load/update&file=control&sound_alert=checkguest&terminal=<?=$arr[web_user][alert_lab_call_terminal];?>";
