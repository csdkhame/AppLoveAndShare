...
//$res[project] = $db->select_query("SELECT $select_order FROM ".TB_transfer_report_all."  where   status = 'CONFIRM'  and airout_time <>'' group by invoice order by  airout_time ASC ");


$res[project] = $db->select_query("SELECT $select_order FROM ".TB_transfer_report_all."  where   status = 'CONFIRM'  and airout_time <>'' group by invoice order by  airout_time ASC  limit 10");
...
/// map cn
 ...
 $row_no = $db->num_rows(TB_transfer_report_all,"id","drivername='".$user_id."'   $find_date and status = 'CONFIRM' and driver_complete  ='0' group by driver_complete,invoice order by driver_complete desc   ");
$row_yes = $db->num_rows(TB_transfer_report_all,"id","drivername='".$user_id."'  $find_date  and status = 'CONFIRM' and driver_complete  ='1' group by driver_complete,invoice order by driver_complete desc ");
...
<span class="font_35" style="font-size:28px; margin-left:15px; width:100%; background-color: #FC9A35; color:#FFFFFF;box-shadow: 1px 1px 5px #999999; border:2px solid #FFFFFF;  border-radius: 5px;"><table width="100%"  border="0" cellspacing="1" cellpadding="1" >
  <tr>
    <td width="100" style="font-size:26px; margin-left:15px;  color:#FFFFFF "><b><center><? echo $daywork;   ?></b></td>
			  ...
			  <a  id="btn-all-<? echo $daywork;?>" class="btn btn-app" style="padding:5px; height:35px; width:29%;font-size:14px; " >
...
///    alert(55);
 ...
 $('.tab-no-<? echo $daywork;?>').show(); 
			  ...
			  <a  id="btn-no-<? echo $daywork;?>" class="btn btn-app" style="padding:5px; height:35px;width:29%;font-size:14px;			  
      ...
      <b>   ไม่สำเร็จ</b>
              </a>   <a  id="btn-yes-<? echo $daywork;?>" class="btn btn-app" style="padding:5px; height:35px; width:29%; font-size:14px;  
...
<? if($i==1){?>
  ...
  <tr>
	...
	<a href="geo://13.694929,100.523987?q=Katina%20Hotel%2C%205%2F61%20Sakdidaj%20Road%2C%20Mueang%20Phuket%2C%2083000%2C%20Thailand" data-rel="external"></a>
...
$arr[remark] = $db->fetch($res[remark]);
  ...
  <td width="80" valign="middle" style="font-size:14px ; font-weight:bold; padding-top:5px; padding-right:5px;">  
  
   <script>
 
$('#btn_chat_<? echo $arr[project][id];?>').click(function(){  
  
 
 	// var url_chat_<? echo $arr[project][id];?> = "load_page.php?name=livechat&file=index&vc=<?= $arr[project][invoice];?>&order=<?= $arr[project][orderid];?>&sv=<?=$arr[project][server];?>&driverid=<?=$user_id?>&chat_from=driver&chat_to=customer&lang=th";
	
	
	
var url_chat_<? echo $arr[project][id];?> = "load_page.php?name=livechat&file=index&vc=<?= $arr[project][invoice];?>&order=<?= $arr[project][orderid];?>&sv=<?=$arr[project][server];?>&driverid=<?=$user_id?>&chat_from=callcenter&chat_to=customer&lang=th";
...
///  $( ".bottom_popup" ).slideToggle();
 ...
 /// $( "#main-sub-loader" ).show();
	...
	$('#load_data_manage_work').load(url_chat_<? echo $arr[project][id];?>); 
...
$('#btn_chat_dv_<? echo $arr[project][id];?>').click(function(){  
...
var url_chat_<? echo $arr[project][id];?> = "load_page.php?name=livechat&file=index&vc=<?= $arr[project][invoice];?>&order=<?= $arr[project][orderid];?>&sv=<?=$arr[project][server];?>&driverid=<?=$user_id?>&chat_from=driver&chat_to=customer&lang=th";
 ...
 /// var url_chat_<? echo $arr[project][id];?> = "load_page.php?name=livechat&file=index&vc=<?= $arr[project][invoice];?>&order=<?= $arr[project][orderid];?>&sv=<?=$arr[project][server];?>&driverid=<?=$user_id?>&chat_from=customer&chat_to=driver&lang=th";
...
<a id="btn_chat_dv_<? echo $arr[project][id];?>"> 
  
<!---<a href="chat.php?name=livechat&file=index&vc=<?= $arr[project][invoice];?>&sv=<?=$arr[project][server];?>" target="_blank">
-->
  
  
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td style="font-size:22px ; color: #009999  ; color:#444444 " width="30">&nbsp;<i class="fa  fa-comment" style=" color: #009999  "></i></td>
        <td style="font-size:14px ; font-weight:bold;color: #009999 ">&nbsp;แชท</td>
      </tr>
    </table> </a>
...
$('#btn_chat_agent_<? echo $arr[project][id];?>').click(function(){  
...
var url_chat_<? echo $arr[project][id];?> = "load_page.php?name=livechat&file=index&vc=<?= $arr[project][invoice];?>&order=<?= $arr[project][orderid];?>&sv=<?=$arr[project][server];?>&driverid=<?=$user_id?>&chat_from=agent&chat_to=customer&lang=th";
...
///   $( ".bottom_popup" ).slideToggle();
  $( "#load_data_manage_work_show" ).show();
 
 	var url_chat_<? echo $arr[project][id];?> = "load_page_show.php?name=view&file=show&id=<?=$arr[project][id];?>&day=<?=$daywork;?>&carno=<?=$arr[project][carno];?>&cartype=<?= $arr[project][cartype];?>&car_list=<?=$i;?>&sv=<?=$arr[project][server];?>";
/// $( "#load_data_manage_work" ).toggle();
...
<div class="btn-startwork"  id="confirm_start_work_<?=$arr[project][id];?>"><?echo $arr[project][airout_time];?><font class="font_20">&nbsp;&nbsp; จัดการงาน   </font>

<font  color="#FFFF66">   </font></div> 

 
              
<div class="btn-startwork" style="background-color: #666666"  id="confirm_new_work_<?=$arr[project][id];?>" ><?echo $arr[project][airout_time];?><font class="font_20">&nbsp;&nbsp; รับทราบงาน   </font>
...
<div  style=" font-size:20px; font-weight:bold; background-color:#FFFFFF; padding:1px;border-radius: 0px; color:367FA9; margin:5px;margin-top:20px; display:nones ">
...
<? }   else { ?> </font><font style="font-size:13px "><? echo "&nbsp;ยังไม่มี"; } ?></font>			 
 ...
 <div id="load_time_pickup_<?=$arr[project][id]?>"> 
				...
				style="color:<? 	if($arr[project][driver_pickup]==1){ echo $clock_color;} else {  echo $no_clock_color; }?>" >
				        ...
				        <div  style="font-size:16px; margin-top:-20px; margin-left:25px;">      
			<? 	if($arr[project][driver_pickup]==1){ ?><font color="<?=$time_color;?>"><b>
			...
			<? }   else { ?> </font><font style="font-size:13px "><? echo "&nbsp;ยังไม่มี"; } ?></font>
                ...
                <div class="timeline-header"  style="padding-top:10px; padding-bottom:5px; "><a ><font >รับแขก</font></a> <span class="font-16" style="margin-top: 2px; position:absolute ; font-size:20px"><B >&nbsp;&nbsp;<? echo $arr[project][airout_time];?></B></span>
...
<?
	  ...
	  ?>
            ...
            <li class="<? if($arr[project][driver_complete]=="1"){ ?>tab-yes-<? echo $daywork;?><? } ?><? if($arr[project][driver_complete]<>"1"){ ?>tab-no-<? echo $daywork;?><? } ?>" >
              ...
              <div class="timeline-item" style="margin-left:40px; margin-right: 0px;background-color:FFFFFF;" id="show_price_<?= $arr[project][id];?>">
				...
				<font color="#666666" style="font-size:16px"><a >รายรับ</a></font> <B></B> &nbsp;<font color="<?=$price_color?>"  style="font-size:16px; margin-top:-25px;"><?=$price_text?>&nbsp;</font >
