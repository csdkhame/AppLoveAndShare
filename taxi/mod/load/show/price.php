...
$data_now="where drivername='".$user_id."' and  id='".$_GET[bookid]."' ";

// echo $date_finish;


$data_all= "where drivername='".$user_id."'   and transfer_date  BETWEEN  '".$date_start."' and '".$date_finish."' and status = 'CONFIRM' and driver_pickup > 0";

$data_today_yes= "where drivername='".$user_id."'   and transfer_date  ='".$date_finish."' and status = 'CONFIRM' and driver_pickup > 0";

$data_today_no= "where drivername='".$user_id."'   and transfer_date  ='".$date_finish."' and airout_time > '".$nowtime."' and status = 'CONFIRM' and driver_pickup < 1 and airout_time <>'' group by invoice order by  airout_time ASC ";

$allday= $db->num_rows('transfer_report_all',"id","drivername='".$user_id."'   and transfer_date  ='".$date_finish."' and airout_time > '".$nowtime."' and status = 'CONFIRM' and driver_pickup < 1 and airout_time <>'' group by invoice order by  airout_time ASC "); 

///  $data_now="where drivername='".$user_id."'  and status = 'CONFIRM'";
...
$pay_today_yes= $row['SUM(car_price)'];
  ...
  $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$query = "SELECT invoice, SUM(car_price) FROM transfer_report_all  where drivername='".$user_id."'   and transfer_date  ='".$date_finish."' and airout_time > '".$nowtime."' and status = 'CONFIRM' and driver_pickup < 1 and airout_time <>'' group by invoice order by  airout_time ASC limit $allday"; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
